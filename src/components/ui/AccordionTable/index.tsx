...
export default function AccordionTable({ isExpand, title, tableTitle, tableItems, tableItemsClass, tableItemsClassTitle, includedList }: AccordionTableProp) {
  ...
  return (
    <div className='bg-slate-100 rounded-3xl overflow-hidden'>
      <div onClick={handleClick} className='cursor-pointer items-center flex justify-between text-lg lg:text-2xl font-semibold p-5'>
        <h3 className='font-montserrat uppercase tracking-wide'>{title}</h3>
        <div className='text-base opacity-70'>{isOpen ? <span>collapse</span> : <span>expand</span>}</div>
      </div>
      <div className={isOpen ? 'block p-5 pt-0' : 'hidden p-5 pt-0'}>
        ...
        <div className='bg-white rounded-3xl overflow-hidden'>
          ...
          {tableItems.map(({ diveDestination, totalDive, price, program, duration, minimumPerson }, index) => {
            return (
              <div key={index} className='flex gap-1 lg:gap-0 flex-wrap py-4 px-2 border-t-2 hover:bg-slate-50 text-sm'>
                ...
                <div className={tableItemsClass}>{price}</div>
              </div>
            )
          })}
        </div>
